#import "OrientationLocker.h"

#ifdef RCT_NEW_ARCH_ENABLED
#import "RNOrientationLockerSpec.h"
#endif

@implementation OrientationLocker
{
#if (!TARGET_OS_TV)
...
- (void)lockToOrientation:(UIInterfaceOrientation) newOrientation usingMask:(UIInterfaceOrientationMask) mask  {
    ...
    [OrientationLocker setOrientation:mask];
...
RCT_EXPORT_METHOD(getAutoRotateState:(RCTResponseSenderBlock)callback)
{
#if (!TARGET_OS_TV)
    callback(@[@YES]);
#endif
...
RCT_EXPORT_BLOCKING_SYNCHRONOUS_METHOD(getInitialOrientation)
{
#if (!TARGET_OS_TV)
    UIInterfaceOrientation orientation = [self getInterfaceOrientation];
    NSString *orientationStr = [self getOrientationStr:orientation];
...
RCT_EXPORT_METHOD(lockToLandscape)
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([orientationStr isEqualToString:@"LANDSCAPE-RIGHT"]) {
            [OrientationLocker setOrientation:UIInterfaceOrientationMaskLandscape];
...
{
    return std::make_shared<facebook::react::NativeOrientationLockerSpecJSI>(params);
}
#endif
